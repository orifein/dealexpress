...
2. **Telegram**: run `node scripts/telegram-post.js --max 1` scoped to this deal (or let it pick up the newly-added file naturally — it already tracks what's posted via `content/telegram/posted.json`, so re-runs are safe *only if step 1a was actually done*). Requires `TELEGRAM_BOT_TOKEN` in the environment. Note the default `--max` is 5 and it posts oldest-unposted-first (round-robin by store) — if you only want to post the deal you just integrated (not sweep the whole backlog), scope it deliberately rather than running the bare script when a backlog of older unposted deals exists.
