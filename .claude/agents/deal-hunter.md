...
2. Search for real, currently-live products (WebSearch + WebFetch on results). Check `content/deals/*.json` first (titles, itemId fields) to avoid a near-duplicate. Source up to **7-8 candidates per run**, aiming for this store mix each run: **2-3 Amazon, 2-3 AliExpress, 1 iHerb, 2 SHEIN**. This is a target, not a quota — if a store genuinely has nothing verifiable this run, shift the difference to another store rather than padding with a weak candidate. Quality still beats volume: hand back fewer than 7-8 if that's all you can verify.
