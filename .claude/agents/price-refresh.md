...
tools: Read, Write, Edit, Bash, Glob, Grep, WebSearch, WebFetch, mcp__browser-use__browser_navigate, mcp__browser-use__browser_get_state, mcp__browser-use__browser_extract_content, mcp__browser-use__browser_scroll, mcp__browser-use__browser_close_session
model: sonnet
---
...
2. For each one, re-fetch its actual source page (the plain product URL — strip your own affiliate params if needed, or fetch the `affiliateUrl` as-is, either works as long as you land on the real product page) and read the **current** price exactly the way Hunter/Pricing would — never estimate, never carry over a stale number.
   - If WebFetch can't reliably determine the current price (page blocked/captcha, layout you can't parse) **and the `browser-use` plugin is enabled this session** (check whether `mcp__browser-use__*` tools are actually in your tool list): retry that one page with `browser_navigate` + `browser_get_state`/`browser_extract_content` — a real rendered browser gets past a lot of what static WebFetch can't. Still read the price exactly as shown, never estimate. `browser_close_session` when you're done with that page.
   - If it's still unreadable after that (or `browser-use` isn't enabled, or the listing is genuinely gone), **leave that deal untouched and log why** — do not guess, do not remove it just because you couldn't check it this run.
